...
func (r *Recorder) LocalMetricsFlushCounts(ms metricsSummary) {
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalCounters), []string{"metric_type:counter"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalGauges), []string{"metric_type:gauge"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalLocalHistograms), []string{"metric_type:local_histogram"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalLocalSets), []string{"metric_type:local_set"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalLocalTimers), []string{"metric_type:local_timer"}, 1.0)
}
...
func (r *Recorder) GlobalMetricsFlushCounts(ms metricsSummary) {
	...
	// again
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalGlobalCounters), []string{"metric_type:global_counter"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalHistograms), []string{"metric_type:histogram"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalSets), []string{"metric_type:set"}, 1.0)
	r.Statsd.Count("worker.metrics_flushed_total", int64(ms.totalTimers), []string{"metric_type:timer"}, 1.0)
}
...
func (r *Recorder) FlushDuration(start time.Time, part string) {
	r.Statsd.TimeInMilliseconds("flush.total_duration_ns", DurationSinceStart(start), []string{fmt.Sprintf("part:%s", part)}, 1.0)
}

// FlushMetricCount tracks the number of metrics posted.
func (r *Recorder) FlushMetricCount(count int) {
	r.Statsd.Gauge("flush.post_metrics_total", float64(count), nil, 1.0)
}

// GCStats tracks various Go GC metrics.
func (r *Recorder) GCStats(mem *runtime.MemStats) {
	r.Statsd.Gauge("mem.heap_alloc_bytes", float64(mem.HeapAlloc), nil, 1.0)
	r.Statsd.Gauge("gc.number", float64(mem.NumGC), nil, 1.0)
	r.Statsd.Gauge("gc.pause_total_ns", float64(mem.PauseTotalNs), nil, 1.0)
}
...
func (r *Recorder) PluginFlushDuration(start time.Time, plugin string) {
	r.Statsd.TimeInMilliseconds(fmt.Sprintf("flush.plugins.%s.total_duration_ns", plugin), DurationSinceStart(start), nil, 1.0)
}
...
func (r *Recorder) PluginFlushErrorCount(plugin string) {
	r.Statsd.Count(fmt.Sprintf("flush.plugins.%s.error_total", plugin), 1, nil, 1.0)
}
